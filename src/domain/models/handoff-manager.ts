...
import { CollaborativeBeliefs, HandoffOperation, HandoffStatus } from "./collaborative-beliefs";
import { Intention } from "./intention";
import { CollaborativeIntentionFactory } from "./collaborative-intention-factory";
import { IntentionQueue } from "./intention-queue";
import { Messenger, HandoffRequestMessage, HandoffResponseMessage } from "../communication/messenger";
...
export class HandoffManager {
    ...
    evaluateHandoffOpportunity(
        ...
        for (const agent of knownAgents) {
            ...
            if (this.beliefs.shouldInitiateHandoff(
                agent.agentId,
                currentPosition,
                targetPosition,
                agent.position,
    ...
    private async initiateHandoff(
        ...
        try {
            ...
            console.log(`Initiated handoff with agent ${targetAgentId} for parcels ${parcelIds.join(', ')} at position ${meetingPosition.toString()}`);
    ...
    private async handleHandoffRequest(request: HandoffRequestMessage): Promise<void> {
        console.log(`Received handoff request from agent ${request.senderId} for parcels ${request.parcelIds.join(', ')} at position ${request.meetingPosition.toString()}`);
        ...
        if (shouldAccept) {
            ...
            console.log(`Accepted handoff request ${request.requestId} from agent ${request.senderId}`);
        } else {
            // Send rejection
            await this.messenger.sendHandoffResponse(
                request.senderId,
            ...
            console.log(`Rejected handoff request ${request.requestId} from agent ${request.senderId}`);
        }
    }
    ...
    async executeHandoff(handoffId: string, isInitiator: boolean): Promise<boolean> {
        ...
        try {
            if (isInitiator) {
                ...
                console.log(`Putting down parcels ${handoff.parcelIds.join(', ')} for handoff ${handoffId}`);
                ...
                console.log(`Picking up parcels ${handoff.parcelIds.join(', ')} from handoff ${handoffId}`);
