...
import { CollaborativeBeliefs } from "./collaborative-beliefs";
...
export class PlayerHandoffExtension {
    ...
    static initializeHandoffCapabilities(
        ...
        player["_checkAndRecalculateIntentions"] = function(forceRecalculate: boolean = false, justPickedUp: boolean = false): void {
            ...
            if (player["_beliefs"].isCarrying) {
                ...
                if (deliveryPoint?.position) {
                    // Get known agents
                    const knownAgents = Array.from(beliefs.getAgentIntentions()).map(intention => ({
                    ...
                    const totalScore = player["_beliefs"].carriedParcels.reduce(
                        (sum, parcel) => sum + parcel.score.currentValue, 0
                    );
        ...
        player["_run"] = async function(): Promise<void> {
            ...
            if (currentIntention && CollaborativeIntentionFactory.isHandoffIntention(currentIntention)) {
