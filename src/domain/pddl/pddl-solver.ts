...
export class PddlSolver {
    ...
    public async getPlan(agentPosition: Position, targetPosition: Position): Promise<PddlPlan> {
        ...
        const objects = pddlMap.get("objects");
        const init = pddlMap.get("init");
        ...
        const goal = [`(at tile_${targetPosition.toPddlString()})`];
        ...
        const responseCheckUrl = await this.postRequest(
            this._pddlProblem.pddl_domain,
            pddlProblem.toPDDLString(),
        );

        const json = await this.getResult(responseCheckUrl);

        return this.parsePlan(json);
    }
    ...
    private async getResult(responseCheckUrl) {
        ...
        while (true) {
            ...
            if (res.status != 200) {
                throw new Error(
                    `Received HTTP error from ${this.host + res.status} ` + (await res.text()),
                );
            }
    ...
    private async parsePlan(json): Promise<PddlPlan> {
        ...
        // PARSING plan from /package/enhsp-2020/solve
            ...
            const endIndex = lines.findIndex((line) => line.includes("Plan-Length"));
        ...
        for (const line of lines) {
            ...
            const action = line[0].toLowerCase();
            const from_splits = line[1].split("_"); // tile_x_y
            const from = new Position(
                Number.parseInt(from_splits[1]),
                Number.parseInt(from_splits[2]),
            );
            const to_splits = line[2].split("_"); // tile_x_y
            const to = new Position(Number.parseInt(to_splits[1]), Number.parseInt(to_splits[2]));
