...
\begin{itemize}
	...
	\item Resource limitations (e.g., the agent can carry only one parcel at a time),
	\item Team-level priorities (if operating in collaborative mode).
\end{itemize}

Once intentions are selected, the agent invokes a planning function to generate an executable plan $\pi$ — a sequence of actions such as \texttt{move}, \texttt{pickup}, and \texttt{deliver}. The plan is constructed using the goal-directed search A*, constrained by the current beliefs.

\subsection{Communication and Coordination}

In collaborative scenarios, agents utilize DeliverooJS’s messaging system to share critical beliefs. This includes discovered parcel locations, unreachable areas, or intentions (e.g., ``I am picking up parcel at (5,3)''). The belief base is extended to include received messages, which influence both desire generation and intention filtering.
...
\begin{itemize}
	...
	\item \textbf{Ally intentions (collaborative mode):} In team scenarios, allies may share their current goals, planned paths, or discovered parcels. These beliefs are integrated to coordinate pickup strategies and reduce redundancy.
